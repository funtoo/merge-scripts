...
_calculate_src_uri() {
	...
	case ${CATEGORY} in
		kde-apps)
			case ${PV} in
				16.12.2)
					SRC_URI="mirror://kde/Attic/applications/16.12.2/src/${_kmname}-${PV}.tar.xz" ;;
				16.08.3)
					SRC_URI="mirror://kde/Attic/applications/16.08.3/src/${_kmname}-${PV}.tar.xz" ;;
				16.04.3)
					SRC_URI="mirror://kde/Attic/applications/16.04.3/src/${_kmname}-${PV}.tar.xz" ;;
				??.?.[6-9]? | ??.??.[6-9]? )
