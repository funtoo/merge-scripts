...
src_install() {
	...
	doman "${FILESDIR}"/depmod.8 || die
	doman "${FILESDIR}"/insmod.8 || die
	doman "${FILESDIR}"/lsmod.8 || die
	doman "${FILESDIR}"/modinfo.8 || die
	doman "${FILESDIR}"/modprobe.8 || die
	doman "${FILESDIR}"/rmmod.8 || die
