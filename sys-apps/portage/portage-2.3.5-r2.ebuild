...
pkg_postinst() {
	...
	# have compatibility
	if [[ -e ${EROOT}etc/make.conf && ! -L ${EROOT}etc/make.conf && ! -e ${EROOT}etc/portage/make.conf ]]; then
		ln -s "${EROOT}etc/portage/make.conf" "${EROOT}etc/make.conf"
	elif [[ -e ${EROOT}etc/make.conf && ! -L ${EROOT}etc/make.conf && -e ${EROOT}etc/portage/make.conf ]]; then
