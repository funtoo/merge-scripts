...
pkg_setup() {
	...
	if use modules ; then
			...
			kernel_is ge 2 6 32 || die "Linux >=2.6.32 and <3.14 required"
			kernel_is lt 3 14 || die "Linux >=2.6.32 and <3.14 required"
		    linux-mod_pkg_setup
	else
		CONFIG_CHECK+=" ~OPENVSWITCH"
...
pkg_postinst() {
	...
	elog "${PN} built against kernels 3.14 and greater must have kernel built modules"
