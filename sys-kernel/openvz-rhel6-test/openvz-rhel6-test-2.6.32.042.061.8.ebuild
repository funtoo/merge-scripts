...
CKV=2.6.32
OKV=$CKV
OVZ_KERNEL="042stab061"
OVZ_REV="8"
OVZ_KV=${OVZ_KERNEL}.${OVZ_REV}
KV_FULL=${PN}-${PVR}
BRANCH="rhel6-${CKV}-testing"
EXTRAVERSION=-${OVZ_KV}
...
DESCRIPTION="Full Linux kernel sources - RHEL6 kernel with OpenVZ patchset (testing)"
HOMEPAGE="http://www.openvz.org"
MAINPATCH="patch-${OVZ_KV}-combined.gz"
SRC_URI="${KERNEL_URI}
	http://download.openvz.org/kernel/branches/${BRANCH}/${OVZ_KV}/configs/config-${CKV}-${OVZ_KV}.i686
	http://download.openvz.org/kernel/branches/${BRANCH}/${OVZ_KV}/configs/config-${CKV}-${OVZ_KV}.x86_64
	http://download.openvz.org/kernel/branches/${BRANCH}/${OVZ_KV}/patches/$MAINPATCH"
S="$WORKDIR/linux-${CKV}"
