...
src_prepare() {
	...
	# Fixes upstream bug #57657 for 4.8.1.
	# Reported as FL-810 by Juraj Kvasnica
	cat "${FILESDIR}"/gcc-4.8.2-fix-cache-detection.patch | patch -p1 || die "patch fail"

	# Fixes upstream bug #57532 "bogus error with int() & int()"
	# Reported as FL-998 by Frank Homann
	cat "${FILESDIR}"/gcc-4.8.1-bogus-error-with-int.patch | patch -p1 || die "patch fail"
