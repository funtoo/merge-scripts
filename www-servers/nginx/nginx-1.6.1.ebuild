...
syslog systemd userland_GNU vim-syntax"
...
src_prepare() {
	...
	if use syslog; then
		epatch "${SYSLOG_MODULE_WD}"/syslog-${SYSLOG_MODULE_NGINX_PV}.patch
	fi

	if use nginx_modules_http_upstream_check; then
		epatch "${HTTP_UPSTREAM_CHECK_MODULE_WD}"/check_1.5.12+.patch
	fi
